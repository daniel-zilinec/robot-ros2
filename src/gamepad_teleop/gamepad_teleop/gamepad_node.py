...
class GamepadTeleopNode(Node):
    """Publish drive and steering motor commands from a Linux joystick device."""

    def __init__(self) -> None:
        super().__init__('gamepad_teleop_node')

        self.declare_parameter('device_path', '/dev/input/js0')
        self.declare_parameter('topic_drive', '/motor_cmd')
        ...
        self.declare_parameter('max_drive', 1.0)
        ...
        self.topic_drive = str(self.get_parameter('topic_drive').value)
        ...
        self.max_drive = max(0.0, min(1.0, float(self.get_parameter('max_drive').value)))
        ...
        self._pub_drive = self.create_publisher(Float32, self.topic_drive, 10)
        ...
        self.get_logger().info(
            'Gamepad teleop ready: device=%s drive_topic=%s steering_topic=%s axis_drive=%d axis_steering=%d deadman=%d'
            % (
                self.device_path,
                self.topic_drive,
                self.topic_steering,
    ...
    def _on_timer(self) -> None:
        drive = self._axis(self.axis_drive)
        steering = self._axis(self.axis_steering)

        drive = self._apply_deadzone(drive)
        ...
        self._pub_drive.publish(Float32(data=float(drive)))
